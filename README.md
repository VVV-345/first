...
在 [huggingface.co](https://huggingface.co) 创建 Space，SDK 选 **Gradio**。

### 第二步：配置 Secrets

Space → Settings → Secrets，添加：
...
### 第四步：上传向量库（绕过 git 大文件限制）

1. 打开 Space → **Files** 标签
2. 点 **Upload files**
3. 将本地 `qdrant_db/` 整个文件夹拖入上传
4. 等待上传完成，重启 Space

### 第五步：访问

Space 自动构建完成后，访问 `https://huggingface.co/spaces/你的用户名/项目名` 即可使用。
